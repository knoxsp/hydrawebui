# -*- coding: utf-8 -*-
"""
Global configuration file for TG2-specific settings in webui.
...
from webui.lib import app_globals, helpers 
...
base_config.sa_auth.cookie_secret = "00a85b8d-d0dc-498c-94b7-a5f3b4a7c4fe"
...
class ApplicationAuthMetadata(TGAuthMetadata):
    ...
    def authenticate(self, environ, identity):
        user = self.sa_auth.dbsession.query(self.sa_auth.user_class).filter_by(username=identity['login']).first()
        if user and user.validate_password(identity['password']):
            return identity['login']
    def get_user(self, identity, userid):
        return self.sa_auth.dbsession.query(self.sa_auth.user_class).filter_by(username=userid).first()
    def get_groups(self, identity, userid):
        return [r.role_code for r in identity['user'].roles]
    def get_permissions(self, identity, userid):
        return [p.perm_code for p in identity['user'].permissions]
